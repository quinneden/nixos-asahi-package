# Modified copy of nixpkgs/nixos/lib/make-disk-image.nix to allow
# converting ext4 filesystem to btrfs after image build.
# TODO: Allow creating btrfs filesystem directly without needing to convert.
{
  pkgs,
  lib,
  ...
  buildImage = pkgs.vmTools.runInLinuxVM (
    pkgs.runCommand name
      ...
      ''
        ...
        ${lib.optionalString convertToBtrfs ''
          modprobe btrfs

          # Make sure existing ext4 is clean.
          e2fsck -fvy $rootDisk

          # Convert filesystem to btrfs.
          btrfs-convert -L --uuid copy $rootDisk

          # Check for errors.
          btrfs check $rootDisk

          mountPoint="/mnt/btrfs"
          mkdir -p $mountPoint
          mount -t btrfs $rootDisk $mountPoint

          # Remove original filesystem metadata.
