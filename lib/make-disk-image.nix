...
let
  ...
  writePartInfoForAsahiInstaller = ''
    partx "$out/${filename}" -rgo SIZE -b --nr 1 | xargs echo -n > "$out/esp_size"
    partx "$out/${filename}" -rgo SIZE -b --nr 2 | xargs echo -n > "$out/root_size"
  '';
    ...
    pkgs.runCommand name
      {
        ...
        postVM = moveOrConvertImage + createHydraBuildProducts + writePartInfoForAsahiInstaller + postVM;
